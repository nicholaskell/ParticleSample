...
//g_Camera:
CCamera g_Camera;
const int UPDATE_FUNCTION_TIME = 30; // every 30 ms
...
long unsigned int timeGetTime()
{
	...
	printf( "VALUE: %d\n", now.tv_sec, ((now.tv_sec*1000)+now.tv_usec) );
	...
	////if( g_iLastRenderTime > 
	
	////retVal = now.tv_usec+now.tv_sec;
	////printf( "%d and %d\n", now.tv_sec, now.tv_usec );
	//return now.tv_usec/1000000;
}
...
//INIT SYSTEM 3 (RAIN)
	...
	g_ParticleSystem3.LoadTextureFromFile("particle2.png");
...
//INIT SYSTEM 5 ("ENGINE")
	...
	g_ParticleSystem5.LoadTextureFromFile("particle3.png");
...
//INIT SYSTEM 6 (FIRE2)
	...
	g_ParticleSystem6.LoadTextureFromFile("particle1.png");

}

void update(int value)
{
	//printf( "Going into update\n" );
	//gameMgr.Update();
	//handleEvents();
	glutPostRedisplay();
