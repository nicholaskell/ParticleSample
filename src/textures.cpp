...
unsigned char * Get_PNG_Data( const char *szFile, unsigned int *w, unsigned int *h )
{
	...
	lodepng::decode(image, width, height, szFile, LCT_RGBA);

	imageSize = image.size();

	// images are always loading upside down...
	image2 = new unsigned char[imageSize];
	imagePtr = &image[0];
	halfTheHeightInPixels = height / 2;
	heightInPixels = height;
	widthInChars = width * numColorComponents;
	...
	for (unsigned int x = 0; x < width; ++x)
	{
		for (unsigned int y=0; y<height; ++y)
		{
			...
			{
				((GLubyte*)image2)[swap]=image[swap];
				swap++;
			}
		}
	}
...
bool COGLTexture::LoadFromPNG(const char *filename,const char *AlphaMap, bool bMipMaps)
{
	if (! filename)
	{
		printf( "No file name...\n" );
	...
	pbytes = Get_PNG_Data(filename, &nwidth, &nheight);
	if(pbytes==NULL)
	{
	...
	if(bMipMaps)
		gluBuild2DMipmaps(GL_TEXTURE_2D, 4, nwidth, nheight, GL_RGBA, GL_UNSIGNED_BYTE, pbytes);
	else
		glTexImage2D(GL_TEXTURE_2D, 0, 4, nwidth, nheight, 0, GL_RGBA, GL_UNSIGNED_BYTE, pbytes);

	//	the image was copied, so the pointer can be deleted
	delete[] pbytes;

	Width = (unsigned int)nwidth;
	Height = (unsigned int)nheight;
	//m_bMipMapped = mipmap;
	return true;
}
